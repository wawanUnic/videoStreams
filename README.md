...
физической камеры (/dev/video0)

двух виртуальных устройств (v4l2loopback)

OpenCV-обработки

трансляции через ustreamer с низкой частотой кадров (например, 1 FPS)
...
Примечания
exclusive_caps=1 позволяет каждому клиенту получать собственный формат

MJPEG позволяет работать без перекодирования и с минимальной задержкой

Частота кадров ограничивается на этапе ffmpeg и/или в ustreamer через --desired-fps

OpenCV может использоваться для коррекции искажений, наложения overlay, распознавания и т.д.
